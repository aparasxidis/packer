...
ensure_nosuid_var_tmp()
{
	...
	if mount | grep -Eq '\s/var/tmp\s'; then
		if grep -E '^\s*[^#]+\s+/var/tmp\s+' /etc/fstab; then
			if [ -z "$(grep -E '^\s*[^#]+\s+/var/tmp\s+' /etc/fstab | grep -v 'nosuid')" ]; then
				test=passed
			else
				sed -ri '/s/^\s*([^#]+\s+\/var\/tmp\s*\S+\s+)([^#]*)?(\s+[0-9]\s+[0-9])/\1\2,nosuid\3' /etc/fstab
				mount -o remount,noexec,nodev,nosuid /var/tmp
				[ -z "$(grep -E '^\s*[^#]+\s+/var/tmp\s+' /etc/fstab | grep -v 'nosuid')" ] && [ -z "$(mount | grep -E '\s/var/tmp\s' | grep -v nosuid)" ] && test=remediated
			fi
		else
			test=manual
		fi
