...
ensure_nodev_tmp()
{
	...
	if mount | grep -Eq '\s\/tmp\s'; then
		...
		if grep -E '^\s*[^#]+\s+\/tmp\s*' /etc/fstab; then
			if [ -z "$(grep -E '\s\/tmp\s' /etc/fstab | grep -v 'nodev')" ]; then
				test=passed
			else
				sed -ri '/s/^\s*([^#]+\s+\/tmp\s*\S+\s+)([^#]*)?(\s+[0-9]\s+[0-9])/\1\2,nodev\3' /etc/fstab
				[ -z "$(grep -E '\s\/tmp\s' /etc/fstab | grep -v 'nodev')" ] && test=remediated
